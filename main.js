...
let gameItemsCont = Array.from(document.querySelectorAll("div[class^='game-body_circle-container'"));
let tempoV;
let tempoArr = [];
let stateFlag = false;
let score = document.querySelector('.header_value');
let userScore = document.querySelector('.header_value-user');
let houseScore = document.querySelector('.header_value-house');
...
function swipe(flag, arr, slim, tmpArr){
    ...
    // Stage 1 if FALSE. And Go To Stage 2
        ...
        document.querySelector('.choosed-item--com_bg-circle').classList.add('choosed-item--com_bg-circle--s2');
        ...
        setTimeout(()=>{
            document.querySelector('.choosed-item--com_bg-circle').classList.remove('choosed-item--com_bg-circle--s2');
        }, 1000)
        // Create Heading
...
function gameOver(state, hUser1, hCom1){
    ...
    setTimeout(()=>{
        ...
        // document.querySelector('.game-body_big-circle').classList.add('game-body_big-circle--s4');
        // document.querySelector('.game-body_tiny-circle').classList.add('game-body_tiny-circle--s4');
        Array.from(document.querySelectorAll('.game-body_big-circle')).forEach((e)=>{
            e.classList.add('game-body_big-circle--s4');
        });
        Array.from(document.querySelectorAll('.game-body_tiny-circle')).forEach((e)=>{
            e.classList.add('game-body_tiny-circle--s4');
        });
...
function initGame(btn,heading,hUser,hCom,gmovCon){
    ...
    Array.from(document.querySelectorAll('.game-body_big-circle')).forEach((e)=>{
        e.classList.remove('game-body_big-circle--s4');
    });
    Array.from(document.querySelectorAll('.game-body_tiny-circle')).forEach((e)=>{
        e.classList.remove('game-body_tiny-circle--s4');
    });
    gameItemsCont.forEach((e)=>{
...
setScore('init'); // Initialize both scores
